document.addEventListener('DOMContentLoaded', () => {
    ...
    function changeLanguage(lang) {
        ...
        if (langBtn) {
            const svgIcon = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"/></svg>';
            langBtn.dataset.currentLang = lang;
            langBtn.innerHTML = `${svgIcon}<span class="lang-text">${lang.toUpperCase()}</span>`;
        }
    }
            ...
            if (response.ok) {
                ...
                if (Object.hasOwn(errorData, 'errors')) {
                    const errorMsg = currentLang === 'en'
                        ? `Error: ${errorData["errors"].map(e => e["message"]).join(", ")}`
                        : `Hubo un error: ${errorData["errors"].map(e => e["message"]).join(", ")}`;
    ...
    if (video && interactionButton && interactionOverlay) {
        ...
        interactionButton.addEventListener('click', () => {
            ...
            const currentLang = document.body.dataset.language;
            interactionButton.querySelector('span').textContent = currentLang === 'en' ? 'Close' : 'Cerrar';
        });
        ...
        interactionOverlay.addEventListener('click', () => {
            ...
            const currentLang = document.body.dataset.language;
            interactionButton.querySelector('span').textContent = currentLang === 'en' ? 'Interact' : 'Interactuar';
        });
        ...
        function animationLoop() {
            ...
            if (!duration) { requestAnimationFrame(animationLoop); return; }
        ...
        video.addEventListener('click', (e) => {
            e.preventDefault();
            if (video.paused) {
                video.play().catch(() => console.log('Video autoplay no permitido'));
            }
        });
    }
});                                                                                               